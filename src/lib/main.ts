    ...
    set((state, props) => {
      ...
      const delta =
        typeof props.delta === 'number'
          ? props.delta
          : props.delta[dir.toLowerCase() as Lowercase<SwipeDirections>] || defaultProps.delta;
    ...
    set((state, props) => {
      ...
      if (state.swiping && state.eventData) {
        ...
        if (event.timeStamp - state.start < props.swipeDuration) {
          ...
          const onSwipedDir =
            props[`onSwiped${eventData.dir}` as keyof SwipeableDirectionCallbacks];
...
export function swipable(node: HTMLElement, options?: SwipeableProps) {
  ...
  for (defaultKey in defaultProps) {
    if (transientProps[defaultKey] === void 0) {
      (transientProps[defaultKey] as any) = defaultProps[defaultKey];
    }
  }

  const [handlers, attachTouch] = getHandlers(
    (stateSetter) => (transientState = stateSetter(transientState, transientProps)),
    { trackMouse }
  );

  transientState = updateTransientState(transientState, transientProps, previousProps, attachTouch);

  let cleanupTouch = attachTouch(node, transientProps);
